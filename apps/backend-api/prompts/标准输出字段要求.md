...
photo_analysis.observed_meta 必须包含 time、location、weather、evidence 四个字段。time/location/weather 都是字符串；没有明确证据时必须返回空字符串。location 必须适合电视顶栏：国内只写到市+县/区，例如“杭州市西湖区”，没有县/区只写城市；国外只写国家，不要写街区、道路、门牌或景点细节。evidence 是对象，分别说明 time、location、weather 的可见依据；没有依据时写空字符串。
...
narration_options 必须返回 5 组不同的三段式旁白，每组包含 scene_line、handwritten_line、closing_line；closing_line 不能超过 8 个中文字符；selected_narration_index 为 0-4 的整数。
